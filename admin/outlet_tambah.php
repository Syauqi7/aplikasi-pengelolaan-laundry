...
if(isset($_POST['btn-simpan'])){
    $nama   = trim(stripslashes($_POST['nama_outlet']));
    $alamat = stripslashes($_POST['alamat_outlet']);
    $telp   = stripslashes($_POST['telp_outlet']);

    // Cek apakah nama_outlet sudah ada
    $query_check = "SELECT COUNT(*) AS jumlah FROM outlet WHERE nama_outlet = '$nama'";
    $result = ambildata($conn, $query_check);

    if ($result[0]['jumlah'] > 0) {
        // Jika nama_outlet sudah ada, tampilkan alert dan redirect dengan parameter error
        echo "<script>
            alert('Nama Outlet sudah ada! Silakan gunakan nama lain.');
            window.location.href='outlet_tambah.php?crud=false&msg=Nama Outlet sudah ada!&type=error&title=Gagal';
        </script>";
        exit;
    } else {
