...
if(isset($_POST['btn-simpan'])){
    $nama = trim(stripslashes($_POST['nama_paket']));
    $jenis_paket = stripslashes($_POST['jenis_paket']);
    $harga = stripslashes($_POST['harga']);
    $outlet_id = stripslashes($_POST['outlet_id']);

    // Cek apakah nama_paket dengan outlet_id yang sama sudah ada
    $query_check = "SELECT COUNT(*) AS jumlah FROM paket WHERE nama_paket = '$nama' AND outlet_id = '$outlet_id'";
    $result = ambildata($conn, $query_check);

    if ($result[0]['jumlah'] > 0) {
        // Jika sudah ada, tampilkan alert dan redirect dengan parameter error
        echo "<script>
            alert('Nama Paket sudah ada di outlet ini! Silakan gunakan nama lain.');
            window.location.href='paket_tambah.php?crud=false&msg=Nama Paket sudah ada di outlet ini!&type=error&title=Gagal';
        </script>";
        exit;
    } else {
